...
export function tagOf(node) {
  ...
  // Any non-SAP scope (outside, shop floor) wears the same tag: what matters is
  // that it isn't in SAP. The band label names which group it belongs to.
  if ((node.scope ?? "sap") !== "sap") return "Outside SAP";
  return null;
}
